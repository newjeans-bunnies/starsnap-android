...
import com.sns.starsnap.R
import com.sns.starsnap.main.MainActivity
...
object StarSnapNotificationManager {
    ...
    private const val EXTRA_SNAP_ID = "notification_snap_id"
    private const val EXTRA_EVENT_ID = "notification_event_id"
    ...
    fun show(context: Context, message: RemoteMessage) {
        ...
        val requestCode = resolveRequestCode(message.messageId, payload)
        val contentIntent = Intent(context, MainActivity::class.java).apply {
            ...
            payload.snapId?.let { putExtra(EXTRA_SNAP_ID, it) }
    ...
    internal fun resolveRequestCode(messageId: String?, payload: FcmPayload): Int = payload.eventId
        ?.let { "${payload.type}:$it".hashCode() }
        ?: messageId?.hashCode()
        ?: payload.hashCode()
